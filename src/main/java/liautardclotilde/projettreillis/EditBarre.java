...
import static liautardclotilde.projettreillis.Edit3.getId3;
import static liautardclotilde.projettreillis.NewBarre.VerifListeBarrenulle;
/**
 *
 * @author Elève
 */
public class EditBarre extends Projet_Treillis {

    /**
     * Creates new form EditBarre
     */
    public EditBarre() {
        initComponents();
    }
            ...
            if ((verif1==getlisteNoeuds().size())||(verif2==getlisteNoeuds().size())){
                ...
                if (R1==R2){
                    ...
                    for (int i=0;i<getlisteBarre().size();i++){
                        if (getId3()==getlisteBarre().get(i).getnom()){
                            Barre B=new Barre (R1,R2,getlisteBarre().size()+1,getCatalogue(),num);
                            u=u+1;
                            for (int j=0;j<getlisteTT().size();j++){
                                if (verification_croisement_barre_segment(B,getlisteTT().get(j).getSegment1())==1||verification_croisement_barre_segment(B,getlisteTT().get(j).getSegment2())==1||verification_croisement_barre_segment(B,getlisteTT().get(j).getSegment3())==1){
                            ...
                            if (croisement>0){
                                ...
                                if (VerifListeBarrenulle(getlisteBarre())==true){
                                    getlisteBarre().get(i).setNoeudi(R1);
                                    getlisteBarre().get(i).setNoeudf(R2);
                                    getlisteBarre().get(i).settype(getCatalogue(),num);
                                    dispose(); 
                                }else{
                                    u=u+1;
                                    int he=0;
                                    for (int j=0;j<getlisteBarre().size();j++){
                                        if ((getlisteBarre().get(j).getNoeudf()==B.getNoeudf())&&(getlisteBarre().get(j).getNoeudi()==B.getNoeudi())){
                                            new Erreur5().setVisible(true);
                                        }
                                        if (verification_croisement (getlisteBarre().get(j), B)==0){
                                            he=he+1;
                                        }else{ 
                                            kp.add(getlisteBarre().get(j).getnom());
                                        }
                                    }
                                    setKp(kp);
                                    if (he>0){
    ...
    private void newCataAfficher(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_newCataAfficher
        // TODO add your handling code here:
        Afficher_Catalogue_Barre(getCatalogue());
    }//GEN-LAST:event_newCataAfficher
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(EditBarre.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
